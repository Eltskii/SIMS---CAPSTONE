...
<div class="row">
  <div class="col-lg-12">
    <div class="page-header-row">
      <div>
        <h2 class="title">
          Students by Program/Course
          <span class="campus-badge">BSU Bokod</span>
        </h2>
        <p class="text-muted" style="margin:0">View students enrolled per program/course and year level.</p>
      </div>
    ...
    <div class="col-md-12">
      <div class="card">
        <div class="filter-header">
          <h3><i class="fa fa-filter"></i> Filter by Course and Year</h3>
        </div>
        <div class="card-body">
          <div class="filter-row">
            <div class="form-group">
              ...
              <select name="Course" class="form-control">
                ...
                <?php 
                  $mydb->setQuery("SELECT * FROM `course` ORDER BY COURSE_NAME, COURSE_LEVEL");
                  $cur = $mydb->loadResultList();
                  foreach ($cur as $result) {
                    $selected = (isset($_POST['Course']) && $_POST['Course'] == $result->COURSE_NAME.'-'.$result->COURSE_LEVEL) ? 'selected' : '';
                    $displayText = $result->COURSE_NAME . '-' . $result->COURSE_LEVEL;
                    if (!empty($result->COURSE_MAJOR)) {
                      $displayText .= ' (' . $result->COURSE_MAJOR . ')';
                    }
                    echo '<option value="'.$result->COURSE_NAME.'-'.$result->COURSE_LEVEL.'" '.$selected.'>'.$displayText.'</option>';
                  }
                ?>
              </select>
            </div>
...
<!-- Results Section -->
<?php if(isset($_POST['submit']) && isset($_POST['Course'])): ?>
<div class="row">
  <div class="col-xs-12">
    <div class="card">
      <div class="results-header">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          <div>
            <h3><i class="fa fa-users"></i> Students Enrolled per Program/Course and Year</h3>
            <small style="opacity: 0.9;">
              Course/Year: <?php echo $_POST['Course']; ?>
            </small>
          </div>
          <div>
...
<div class="stats-card">
  ...
  <span class="stats-label">Total Students in <?php echo $_POST['Course']; ?></span>
</div>
<?php endif; ?>

<!-- Print Form (Separate form for printing) -->
<form action="printcourseyear.php" method="POST" target="_blank" id="printForm">
...
<div class="row">
  <div class="col-md-8 col-md-offset-2">
    <div class="empty-state">
      ...
      <p>Please select a course and year to generate the student list.</p>
    </div>
  </div>
</div>
<?php else: ?>
...
<div class="row">
  <div class="col-md-8 col-md-offset-2">
    <div class="empty-state">
      ...
      <h3>Ready to View Students by Course?</h3>
      <p>Select a course and year level above to view enrolled students.</p>
    </div>
  </div>
</div>
<?php endif; ?>