...
switch ($action) {
  ...
  // `sched_time`, `sched_day`, `sched_semester`, `sched_sy`, `sched_room`, `COURSE_ID`, `SUBJ_ID`
		...
		$subj_id = isset($_POST['SUBJ_ID']) ? intval($_POST['SUBJ_ID']) : 0;
		$course_id = isset($_POST['COURSE_ID']) ? intval($_POST['COURSE_ID']) : 0;
		$semester = isset($_POST['sched_semester']) ? mysqli_real_escape_string($mydb->conn, $_POST['sched_semester']) : '';
		
		$sql = "SELECT * FROM `tblschedule` 
		WHERE `SUBJ_ID`={$subj_id} AND `COURSE_ID`={$course_id} AND sched_semester='{$semester}'";
