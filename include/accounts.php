	...
	static function userAuthentication($U_USERNAME,$h_pass){
		...
		$row_count = $mydb->num_rows($cur);//get the number of count
		 ...
		 $user_found = $mydb->loadSingleResult();
			...
			// Store department ID for Chairpersons
			if (property_exists($user_found, 'DEPT_ID') && $user_found->DEPT_ID > 0) {
			    $_SESSION['DEPT_ID'] = (int)$user_found->DEPT_ID;
			} else {
			    $_SESSION['DEPT_ID'] = null;
			}

			// Link to instructor/employee record when available
if (property_exists($user_found, 'EMPID')) {
    $_SESSION['EMPID'] = (int)$user_found->EMPID;
} else {
    $_SESSION['EMPID'] = 0;
}

			// Update last login timestamp
			$mydb->setQuery("UPDATE useraccounts SET LAST_LOGIN = NOW() WHERE ACCOUNT_ID = {$user_found->ACCOUNT_ID}");
			$mydb->executeQuery();
	...
	public function update($id=0) {
	  global $mydb;
		...
		// Exclude ACCOUNT_ID from the UPDATE statement (it's the primary key)
